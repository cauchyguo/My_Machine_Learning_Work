...
class J48:
    ...
    def creatTree(self,dataset,labels):
        ...
        # 存储节点的层数
        deep = len(self.labels) - len(attrs) + 1
        # 建立字典存储每层的属性
        if deep not in self.selectAttrbutes.keys():
            self.selectAttrbutes[deep] = []
        self.selectAttrbutes[deep].append(bestAttrLabel)
        ...
        for value in uniqueVals:
            ...
            myTree[bestAttrLabel][value] = self.creatTree\
                    (self.splitDataSet(dataset,bestAttrIdx,value),subLabels)
        # self.myTree = myTree
        return myTree
    ...
    def get_n_deep_attrs(self,n):
        """在特征树字典中选出前n层的特征集合"""
        selectFeatures = []
        for i in range(1,n+1):
            selectFeatures.extend(self.selectAttrbutes[i])
        return set(selectFeatures)

    def a_train(self,labels,n):
        """一次测试"""
        testSet = []  #测试集
        trainSet = [] #训练集
        ...
        self.selectAttrbutes = {}
        mytree = self.creatTree(trainSet,labels)
        count = 0 #统计分类正确率
        for testVc in testSet:
            if self.classify(mytree,self.labels,testVc) == testVc[-1]:
                count += 1
        print("特征树字典：", self.selectAttrbutes)
        print("挑选的前" + str(n) +"层属性为",self.get_n_deep_attrs(n))
        print("训练集数目",len(trainSet),"测试集数目",len(testSet),"正确率",'{:.2%}'.format(count/numTestSet))

    def train_for_n_times(self,datasetname,labels,times,n):
        '''进行n次测试'''
        self.labels = labels[:]
        ...
        for i in range(times):
            self.labels = labels[:]
            print("测试",i+1,'',end='')
            self.a_train(self.labels,n)

if __name__ == '__main__':
    cTree = J48(r'E:\技术报告\任务二\C4.5\venv\Data\absenteeism_at_work.csv')
    labels = ['ID','Reason for absence','Month of absence','Day of the week','Seasons','Transportation expense','Distance from Residence to Work','Service time','Age','Work load Average/day' ,'Hit target',
    ...
    cTree.train_for_n_times("absenteeism_at_work", labels, times=12,n=7)