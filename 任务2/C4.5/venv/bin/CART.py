...
class CART:
    def __init__(self,filename):
        ...
        self.myTree = None
        self.numLabels = set([sample[-1] for sample in self.dataset])
    ...
    def calGini(self,dataset):
        ...
        for key in labelCounts:
            ...
            Gini -= prob ** 2
        return Gini #返回该集合的基尼值

    def chooseBestAttr(self,dataset):
        """选择最好的数据集划分属性,并返回该属性的下标"""
        baseGini = self.calGini(dataset)  # 样本划分前的基尼值
        Ginis = [] #Ginis包含按不同属性分类得到的基尼值
        for i in range(len(dataset[0]) - 1):  # 对样本中的非标签属性一一进行判别,取最小基尼数的属性
            ...
            for value in uniqueAttrsL:
                ...
                prob = len(subDataSet) / (float)(len(dataset))  # 对统计新划得的集合的信息
                newGini += prob * self.calGini(subDataSet)  # 迭代得到该属性下的基尼值
            Ginis.append(newGini)
        return Ginis.index(min(Ginis))#返回最小基尼值对应的属性的下标
    ...
    def train_for_n_times(self,datasetname,labels,n):
        '''进行n次测试'''
        self.labels = labels[:]
        print("数据集：", datasetname,"样本属性数量：",len(self.labels),"样本标签种类：",len(self.numLabels),"样本总数:",len(self.dataset),"评估方法:","自助法")
        for i in range(n):
            self.labels = labels[:]
            print("测试",i+1,'',end='')
...
#     cTree = CART("/home/ubuntu/PycharmProjects/C4.5/venv/Data/absenteeism_at_work.csv")
#     cTree.train_for_n_times("absenteeism_at_work.csv", cTree.Line, n=18)
# if __name__ == '__main__':
#     cTree = CART("/home/ubuntu/PycharmProjects/C4.5/venv/Data/car.csv")
#     labels = ['buying','maint', 'doors', 'peasons','lug_boot','safety']
#     cTree.train_for_n_times("Car",labels,n=12)
if __name__ == '__main__':
    cTree = CART(r'E:\Data\iris.csv')
    labels = ['outlook','temperature','humidity','windy']
    cTree.train_for_n_times("Iris",labels,n=12)